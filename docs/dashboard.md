...
│  └─ *StatsPanel.vue                   四个嵌入式统计块（自拉数据，range 由页面下发）
└─ views/DashboardView.vue              工作台：概览指标 + 统计标签页 + 业务入口工作区
```

## 管理端 UI 结构

工作台按后台运营的日常扫描习惯拆成三层，避免统计面板和业务入口互相抢视觉层级：

- **概览区**：展示当前账号、角色、权限、可见入口和访问状态，作为页面第一屏的低密度摘要。
- **经营数据区**：用一个面板承载订单/财务/用户/打手四个标签页，时间范围（日/月/年）固定在面板右上角；每次只展示一个业务域的关键指标和图表，避免页面纵向堆叠。
- **工作区**：底部展示业务入口、快捷访问、角色与权限概览，适合从数据看板下钻到具体管理页面。
...
- **布局控制**：统计块作为嵌入式内容渲染，由 `DashboardView` 统一提供外层面板、标签页和范围切换；图表卡固定高度并开启坐标轴标签去重，保证 PC 与窄屏下都能稳定扫描。
