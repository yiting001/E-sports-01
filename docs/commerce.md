...
- **C 端商品详情页视觉**：价格区采用战术金渐变面板（大号价格 + 折扣标签 + 划线原价 + 已售），手机端/电脑端价格卡片为固定两行布局（平台名 + 划线原价、现价，折扣角标内置在卡片右上角、不超出边框，文字不换行），有无折扣时卡片高度一致；封面底部渐隐衔接，下单按钮为斜切金色渐变；移动端与 PC 端（双栏 + 右侧吸顶信息卡）均适配。
