...
陪玩下单平台的商品与分类管理。运营在管理端维护「分类」（一级归类，如大红单/护航单）与「商品」（具体陪玩服务单，含标题、封面标语、现价/原价、销量、关联负责客服、上下架状态、排序）；C 端（用户端 `apps/client`）通过免登录公开接口读取启用分类与上架商品。
...
- **关联负责客服**：商品可选关联一名平台用户作为「负责客服」（`serviceAgentId`，可空），为后续「下单拉群只拉指定客服」打基础；管理端表单经 `/commerce/service-agents` 远程搜索候选用户。
