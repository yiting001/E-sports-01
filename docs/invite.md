...
2. **填码绑定**：新用户在邀请页填写好友的 8 位邀请码完成绑定；校验邀请码存在、非本人、一个用户只能被邀请一次（invitee 唯一约束兜底防并发）。
...
└── views/invite/InviteView.vue               # 邀请好友页（码/复制/填码/记录）
```
...
- **配置零硬编码**：奖励方式/券模板/入账金额共 6 个键登记在 `CONFIG_KEYS.invite`（配置中心 group=invite，启动播种默认「不发放」），后台改配置即时生效，绑定时实时读取。
