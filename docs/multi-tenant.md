...
- 新租户在同一数据库事务中创建租户、租户管理员、会员、客服、打手四类基础角色和初始管理员。
...
开发模式未配置时使用 `http://127.0.0.1:5174`；生产构建必须显式配置浏览器可达的
C 端地址，空值、无效 URL 或非 HTTP(S) 协议会关闭跳转并提示配置错误，不回退到管理端自身。
...
自定义角色仍可删除。
租户开通通过 `TenantProvisioningTransaction` 端口进入 TypeORM 事务，租户、四类角色和初始管理员任一写入失败都会整体回滚；并发创建相同编码时，数据库唯一约束错误会转换为业务 409。
初始管理员密码必填且必须为 12-128 位，包含大小写字母、数字和特殊字符，首尾空白不会被自动裁剪通过。
角色授权页读取 `/rbac/roles/grantable-permissions`；子租户只能看到可授予的业务权限，写入端再次拒绝平台权限 ID。

```mermaid
stateDiagram-v2
  [*] --> enabled: 创建并播种基础角色/管理员
