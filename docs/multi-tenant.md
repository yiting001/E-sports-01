...
| 角色编码       | 用途                                     |
| -------------- | ---------------------------------------- |
| `tenant_admin` | 本租户管理；创建时不带任何权限，由平台超管按需授予 |
| `member`       | C 端普通用户                             |
| `service`      | 客服工作台和被授权的订单操作             |
| `booster`      | 打手领域流程授予                         |

启动播种器只为存量租户新建缺失的内置角色，**不会修改、回填或新增任何已存在角色的权限**，
平台超管手动关闭的权限不会因部署/重启而恢复；它只会从非默认租户角色中剔除遗留的平台级权限。租户状态只有 `enabled` 与 `disabled`；默认
...
租户创建不再播种初始管理员账号；平台超管在「用户管理→新建用户」中选择所属租户单独创建，在「角色管理→新建角色」中选择所属租户创建自定义角色；非超管指定其他租户返回 403，绑定跨租户角色返回 403。平台超管还可在「用户管理→编辑用户」中变更用户所属租户：目标租户须存在，用户名/手机号在目标租户须唯一，迁移时自动解绑原租户全部角色并需重新分配目标租户角色；持有平台超管角色的账号不可迁移。
角色的创建、修改、删除、授权与可授予目录 `/rbac/roles/grantable-permissions` 均为 `@PlatformOnly()`，仅默认租户平台超管可用；角色管理写权限码也列入平台级权限，不能授予租户角色。租户管理员只能查看本租户角色，并把本租户已有角色分配给本租户用户（跨租户角色 403）。
...
- 平台目录与角色管理写接口 `PlatformOnly`；
- 新建租户的 `tenant_admin` 无权限；播种器不回填存量角色权限、只新建缺失角色、剔除租户角色平台级权限；
