...
- **接单大厅预估到手**：打手订单卡片采用「订单金额 / 可得佣金」紫色双栏横幅布局（已结算显示「到手佣金」），下方带图标信息行展示数量/大区、游戏昵称、游戏 ID（接单前显示“接单后显示”）、账号、备注与备注图片（复用 `RemarkMediaGallery`，点击图片全屏预览、不触发卡片跳转；备注及媒体接单后才由后端下发，大厅接单前不展示），底部展示「¥x 入账」与渐变圆角主操作按钮；点击卡片进入待接订单详情（详情页不再跳转商品详情）。佣金优先使用后端下发的订单费率，未下发时回退打手自身等级费率（`boosterApi.mine()` 的 `commissionRateBp`）计算，保证列表与详情均能展示可得佣金；`showCommission` 通过 `withDefaults` 声明默认 `true`（Boolean prop 缺省会被 Vue 强转为 `false`，不能依赖 `??` 兜底）。
