...
- **打手完整入驻表单**：登录用户填写姓名、性别、接单区服（选项由管理端「打手管理 → 区服配置」维护，随 `GET /booster/mine` 的 `serviceRegionOptions` 下发，至少选 1 项）、自我介绍、联系方式，可选上传一张材料图和填写邀请码；未申请 / 已驳回可提交，审核中 / 已通过只读展示。顶部公告卡片由配置中心 `booster.onboardingNoticeImage`（图片）与 `booster.onboardingNoticeText`（文本，换行分行展示，与主页公告独立）下发，驳回时保留资料并显示理由。
- **打手订单跳商品详情**：打手订单详情页顶部商品快照块可点击，跳转 `/products/:id` 查看服务内容（商品已下架时详情页展示「商品不存在或已下架」）。
- **打手「我的资金」**：「我的」页打手身份下展示资金面板（沿用接单大厅订单卡电竞风：紫色渐变横幅+闪电徽章突出可用余额，下方网格按资金金色/结算粉色分组展示保证金/冻结金额/已交罚款/累计、本月、上月结算），数据来自 `GET /booster/funds/mine` 只读聚合，加载失败可重试。
...
- **接单大厅检索**：打手大厅支持订单号/商品名搜索以及全部、手机端、电脑端筛选，当前筛选下继续分页与自动刷新；详情页覆盖加载、失败重试、订单消失、上下线门禁和并发接单失败状态；详情页在商品快照下方静默加载商品公开信息（`commerceApi.getProduct`），展示封面副标语（`coverSub`）与商品分类，商品下架或删除时降级隐藏、不影响订单展示。
- **接单大厅预估到手**：打手订单卡片采用「订单金额 / 可得佣金」紫色双栏横幅布局（已结算显示「到手佣金」），下方带图标信息行展示数量/大区、游戏昵称、游戏 ID（接单前显示“接单后显示”）、账号、备注与备注图片（复用 `RemarkMediaGallery`，点击图片全屏预览、不触发卡片跳转；备注文字接单前直接可见，备注图片接单后才由后端下发），商品标题与下发时间之间展示商品封面副标语（`productCoverSub`，两行截断，便于接单前了解订单内容），底部展示「¥x 入账」与渐变圆角主操作按钮；点击卡片进入待接订单详情（详情页不再跳转商品详情）。佣金优先使用后端下发的订单费率，未下发时回退打手自身等级费率（`boosterApi.mine()` 的 `commissionRateBp`）计算，保证列表与详情均能展示可得佣金；`showCommission` 通过 `withDefaults` 声明默认 `true`（Boolean prop 缺省会被 Vue 强转为 `false`，不能依赖 `??` 兜底）。
...
- 主图在稳定 1:1 容器内以 `contain` 完整展示，图片上不叠加任何渐变遮罩；点击后继续以 `contain` 查看原图；可点遮罩、关闭图标或按 Esc 退出。加载失败回退默认徽标，不允许图片固有比例撑开页面。
