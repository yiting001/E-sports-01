...
- **商品详情展示**：游客可访问 `/products/:id`；主图在稳定方形容器内完整展示且可点击预览原图，封面主副标语移到图片下方的独立介绍区，同时展示手机端/电脑端价格与四项服务保障。富文本详情经 DOMPurify 净化后渲染；加载失败可重试，封面失败回退默认徽标。
...
- **接单大厅预估到手**：打手订单卡片采用「订单金额 / 可得佣金」双栏横幅布局（已结算显示「到手佣金」），下方按行展示单号、数量/大区、游戏 ID（接单前显示“接单后显示”）、账号与备注，底部展示「¥x 入账」与主操作按钮；待接订单详情页显示订单金额+预估到手，商品块可点击跳转商品详情查看服务内容。佣金按后端下发的当前打手等级费率计算。
