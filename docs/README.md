...
| 消息通知         | [notify.md](./notify.md)                           | 订单微信小程序/公众号推送（打手绑定、字段映射配置）+ C 端/管理端新订单、新消息语音播报                       |
...
| 支付网关/计全付  | [payment-gateway.md](./payment-gateway.md)         | 支付方式与执行网关解耦（策略+适配器），计全付微信扫码/JSAPI/退款接入、回调解析、同步跳转 `/pay/return` 与管理端「支付配置」页 |
