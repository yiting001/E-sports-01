...
- 备注附件：下单备注支持上传图片/视频（`RemarkMediaUploader`，复用 `/upload/self` 自助上传，最多 `ORDER_LIMITS.remarkMediaMax` 个），订单固化 `remarkMedia` jsonb 快照，详情页/打手订单详情/管理端抽屉以缩略图展示（`RemarkMediaGallery`）；接单大厅列表/详情接单前不下发备注媒体（备注文字保留下发供接单前判断）
...
- 接单大厅（C 端打手身份）：分页浏览待接单订单，支持按订单号/商品名搜索以及“全部、手机端、电脑端”服务区服筛选，顶部可自主上线/下线；提供手动刷新，页面可见时每 5 秒按当前筛选自动刷新并在失败时保留旧列表。移动端刷新按钮声明 `touch-action: manipulation`，防止连续触控被浏览器解释为双击缩放，同时保留页面滚动和双指缩放。点卡片进入大厅详情页（`/hall/:id`），可见商品、区服、金额、备注和下发时间，商品块可点击跳转商品详情查看服务内容，账号信息继续隐藏；大厅列表与详情按当前打手等级费率下发预估口径的 `commissionRateBp`（待接订单未结算时），C 端据此展示「预估到手」金额，实际佣金仍以完成结算时费率为准；只有上线且满足目录、实名、押金和区服门禁的打手能接单
...
接单大厅在 `toHallOrderView` 中清空 `accountInfo`、`gameAccountId`、`gameTextId`、`remarkMedia`（备注媒体接单后才对打手可见，备注文字保留），并注入 `productCoverSub`（商品当前封面副标语，大厅列表批量去重查商品，商品删除回退空串）；区服和打手安排方式仍可用于接单判断。订单详情中的实际打手名称和锁定打手名称均为快照，名称按钮只在存在对应用户 ID 时跳转打手主页。
...
| GET  | `/api/order/hall` / `/api/order/hall/:id`                 | 打手角色                                  | 待接单大厅列表支持 `?keyword&serviceRegion=delta-mobile\|delta-pc&page&pageSize`；详情继续隐藏账号字段；未结算订单的 `commissionRateBp` 为当前打手等级费率（预估口径）                                                                                                               |
