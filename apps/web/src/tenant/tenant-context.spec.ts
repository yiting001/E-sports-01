...
describe('管理端租户上下文', () => {
  ...
  it('跨标签页只把当前租户的分区令牌键识别为当前会话', () => {
    tenantContext.setCode('tenant-one');

    expect(
      tokenStorage.isCurrentTenantTokenKey(
        `${STORAGE_KEYS.accessToken}.tenant-one`
      )
    ).toBe(true);
    expect(
      tokenStorage.isCurrentTenantTokenKey(
        `${STORAGE_KEYS.refreshToken}.tenant-one`
      )
    ).toBe(true);
    expect(
      tokenStorage.isCurrentTenantTokenKey(
        `${STORAGE_KEYS.accessToken}.tenant-two`
      )
    ).toBe(false);
    expect(tokenStorage.isCurrentTenantTokenKey(STORAGE_KEYS.accessToken)).toBe(
      false
    );
    expect(tokenStorage.isCurrentTenantTokenKey(null)).toBe(false);
  });
});
