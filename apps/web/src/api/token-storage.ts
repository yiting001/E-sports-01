import { DEFAULT_TENANT_CODE, type TokenPair } from '@app/contracts';
...
export const tokenStorage = {
  ...
  isCurrentTenantTokenKey(key: string | null): boolean {
    if (!key || tenantContext.entryError.value) {
      return false;
    }
    return (
      key === tenantStorageKey(STORAGE_KEYS.accessToken) ||
      key === tenantStorageKey(STORAGE_KEYS.refreshToken)
    );
  },
  captureSession,
  isCurrentSession,
