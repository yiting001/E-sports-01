 ...
 *   开启计全付时替换为对应的计全付驱动；
 * - 提现：受 wallet.payout.gateway 控制，开启计全付时支付宝/微信零钱提现均走计全付转账驱动。
...
export class PaymentGatewayService {
  ...
  /** 提现渠道：用户选择的支付宝/微信零钱按提现网关开关映射为官方直连或计全付转账。 */
  async resolvePayoutProvider(provider: PayoutProvider): Promise<PayoutProvider> {
    if (provider === PayoutProvider.Alipay || provider === PayoutProvider.Wechat) {
      if (await this.isJqf(CONFIG_KEYS.wallet.payoutGateway)) {
  ...
  private async isJqf(key: string): Promise<boolean> {
    const gateway = await this.config.getString(key, PaymentGateway.Official);
